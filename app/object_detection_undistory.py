...
class FusionLidarCameraNode:
    def __init__(self) -> None:
        ...
        self.distortion_model = str(params.get("distortion_model", "fisheye"))
        self.R_rect = np.asarray(params.get("R_rect", np.eye(3, dtype=np.float64)), dtype=np.float64)
        self.fisheye_balance = _cfg_get(cfg, "fisheye_balance", 0.0, float)
        self.pinhole_alpha = _cfg_get(cfg, "pinhole_alpha", 0.0, float)
        ...
        self.cluster_z_grid_size = _cfg_get(cfg, "cluster_z_grid_size", 0.4, float)
        ...
        self.max_lidarimage_delay = _cfg_get(cfg, "max_lidarimage_delay", 1.0, float)
    ...
    def _ensure_undistort_maps(self, width: int, height: int) -> np.ndarray:
        ...
        rospy.loginfo(
            "object detection undistort map cached: size=%dx%d model=%s",
            int(width),
            int(height),
    ...
    def _process_job(self, job: dict) -> None:
        ...
        try:
            image_raw = camera_handle._ros_image_to_cv2_fallback(image_msg)
            image, K_undist = self._undistort_image_cached(image_raw)
        except Exception as exc:
        ...
        h, w = image.shape[:2]
        xyz_proj, uv, _ = points_project.project_lidar_to_image(
            xyz_lidar=xyz,
