...
class FusionLidarCameraNode:
    def __init__(self) -> None:
        ...
        self.cluster_radius_quantile = float(rospy.get_param("~cluster_radius_quantile", 0.9))
        ...
        rospy.loginfo(
            "fusionLidarCamera ready: image=%s, points=%s",
            self.topic_image,
    ...
    def cmd_callback(self, msg: String) -> None:
        ...
        if task == "follow":
            ...
            self.run = NOTASK
        rospy.loginfo("Received cmd: caption=%s task=%s run=%d", self.caption, task, self.run)
    ...
    def _send_follow_goal(self, center_xy: List[float], surface_xy: List[float]) -> None:
        ...
        try:
            self.tf_listener.waitForTransform(self.goal_frame, self.base_frame, rospy.Time(0), rospy.Duration(0.2))
            trans, rot = self.tf_listener.lookupTransform(self.goal_frame, self.base_frame, rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException) as exc:
            rospy.logwarn(
                "skip follow goal: transform %s <- %s unavailable: %s",
                self.goal_frame,
                self.base_frame,
                str(exc),
            )
        ...
        goal_map_x = float(trans[0] + cos_yaw * goal_xy[0] - sin_yaw * goal_xy[1])
        goal_map_y = float(trans[1] + sin_yaw * goal_xy[0] + cos_yaw * goal_xy[1])
        yaw_map = base_yaw + yaw_rel
