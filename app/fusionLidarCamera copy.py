...
class FusionLidarCameraNode:
    def __init__(self) -> None:
        ...
        self.mask_dilate_px = int(rospy.get_param("~mask_dilate_px", 3))
    ...
    def synced_callback(self, image_msg: Image, cloud_msg: PointCloud2) -> None:
        ...
        try:
            ...
            image_fov_xyz = xyzuv_inside[:, :3]
            self.pub_image_fov_points.publish(self._build_cloud_xyz(cloud_msg.header, image_fov_xyz))
            ...
            cv2.putText(
                debug,
                (
                    f"swap_uv={self.swap_uv} in={inside_ratio:.2f} "
                    f"fov={image_fov_xyz.shape[0]} obj={object_xyz.shape[0]} "
                    f"depth_med={payload['depth_z_median_m']}"
                ),
                (15, 30),
            ...
            rospy.loginfo(
                "fusion done: caption='%s' swap_uv=%s inside=%.3f fov=%d obj=%d depth_med=%s frame=%s",
                self.caption,
                str(self.swap_uv),
                inside_ratio,
                image_fov_xyz.shape[0],
                object_xyz.shape[0],
