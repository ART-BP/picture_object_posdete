...
class FusionLidarCameraNode:
    ...
    def __init__(self) -> None:
        ...
        self.max_infer_fps = self._cfg_get(cfg, "max_infer_fps", 0.0, float)
    ...
    def _process_job(self, job: dict) -> None:
        ...
        stage_times_ms.append(("read_xyz", (time.perf_counter() - t_read_cloud) * 1000.0))
        if xyz.shape[0] == 0:
        ...
        stage_times_ms.append(("project_points", (time.perf_counter() - t_project) * 1000.0))
        if xyz_proj.shape[0] == 0:
            rospy.logwarn_throttle(2.0, "no projected points inside image")
            finish("drop_no_projected")
            return
